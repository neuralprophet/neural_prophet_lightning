...
def get_parameters(df, dataset_name, frequencies, method, usecase):
    '''
    Args:
        df: dataset with data
        dataset_name: name of dataset
        frequencies: frequencies for time series
        method: str, 'onestep' or 'multistep'
        usecase: name of the usecase

    Returns:
        Parameters for the model: n_lags, frequency and n_forecasts.
    ...
    params = {"n_lags": n_lags, "freq": freq, "n_forecasts": n_forecasts}
    return params
...
def benchmark(model, method, df, params, one_dataset_metrics, n_epochs):
    ...
    try:
        if model == "LSTM":
            m = LSTM(n_lags=n_lags, n_forecasts=n_forecasts, learning_rate=0.01, epochs=n_epochs)
        elif model == "NP":
            m = NeuralProphet(n_lags=n_lags, n_forecasts=n_forecasts, learning_rate=0.01, epochs=n_epochs)
        elif model == "DeepAR":
            m = DeepAR(
                n_lags=int(n_lags), n_forecasts=n_forecasts, learning_rate=0.01, epochs=n_epochs, auto_lr_find=False
            )
        elif model == "NBeats":
            m = NBeats(n_lags=int(n_lags), n_forecasts=n_forecasts, learning_rate=0.01, epochs=n_epochs)
        elif model == "TFT":
            m = TFT(n_lags=int(n_lags), n_forecasts=n_forecasts, learning_rate=0.01, epochs=n_epochs)
...
def libra(n_datasets, datasets, frequencies, method, n_epochs, usecase, save_res=True):
    '''
    Args:
        n_datasets: number of time series to use
        datasets: dataset from the usecase
        frequencies: frequencies of time series
        method: str, 'onestep' or 'multistep'
        n_epochs: number of epochs for training of all models
        usecase: name of the usecase
        save_res: bool, whether to save results or not

    Returns: metrics dictionary and save the results
